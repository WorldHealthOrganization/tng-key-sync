...
public class AddCertificateStepTest {
    ...
    void testCertificatesAreAdded() throws Exception {
        ...
        X509Certificate certificateUpload = CertificateTestUtils.generateCertificate(keyPair, TEST_CERT_COUNTRY_CODE, "Test Cert UPLOAD");
        X509Certificate certificateAuth = CertificateTestUtils.generateCertificate(keyPair, TEST_CERT_COUNTRY_CODE, "Test Cert AUTH");
        X509Certificate certificateCsca = CertificateTestUtils.generateCertificate(keyPair, TEST_CERT_COUNTRY_CODE, "Test Cert CSCA");
        X509Certificate certificateDsc = CertificateTestUtils.generateCertificate(keyPair, TEST_CERT_COUNTRY_CODE, "Test Cert DSC");

        ImportJobContext context = new ImportJobContext();

        addCertificateStep.exec(context, Base64.getEncoder().encodeToString(certificateUpload.getEncoded()), "UPLOAD");
        addCertificateStep.exec(context, Base64.getEncoder().encodeToString(certificateAuth.getEncoded()), "AUTHENTICATION");
        ...
        for (ImportJobContext.CertificateEntry certificateEntry : context.getParsedCertificates()) {
            switch (certificateEntry.getCertificateType()) {
                case DSC -> checkParsedCertificateEntry(certificateEntry, certificateDsc, ImportJobContext.CertificateType.DSC);
                case AUTHENTICATION -> checkParsedCertificateEntry(certificateEntry, certificateAuth, ImportJobContext.CertificateType.AUTHENTICATION);
                case CSCA -> checkParsedCertificateEntry(certificateEntry, certificateCsca, ImportJobContext.CertificateType.CSCA);
                case UPLOAD -> checkParsedCertificateEntry(certificateEntry, certificateUpload, ImportJobContext.CertificateType.UPLOAD);
            }
        }
    }

    void checkParsedCertificateEntry(ImportJobContext.CertificateEntry certificateEntry, X509Certificate x509Certificate,
        ...
        Assertions.assertEquals(certificateUtils.convertCertificate(x509Certificate), certificateEntry.getParsedCertificateHolder());
